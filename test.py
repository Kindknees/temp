...
def cli():
    ...
    parser.add_argument("-d", "--dir", type=str, help="change the directory used for storing environment + data", default="")
    parser.add_argument("-n", "--name", type=str, default="untitled")
    parser.add_argument("-la", "--load_agent", type=str, default="", help="load a learned agents parameters to continue training")
    ...
    parser.add_argument("-ns", "--nb_steps", type=int, default=100, help="total number of agent steps")
    parser.add_argument("-ev", "--eval_steps", type=int, default=50, help="the number of steps between each evaluation")
    ...
    parser.add_argument("-a", "--agent", type=str, default="sacd_emb", 
                       choices=["sac", "sac2", "sacd", "sacd_emb", "sacd2", "isacd_base", "isacd_emb", "dsacd_emb", "dqn", "dqn2", "ppo", "ippo", "dppo"])
    ...
    parser.add_argument("-u", "--update_start", type=int, default=2, help="updating the agent starts after *n* x batch_size")
    # SMAAC specific
    parser.add_argument("-r", "--rule", type=str, default="c", choices=["c", "d", "o", "f"], help="low-level rule (capa, desc, opti, fixed)")
    parser.add_argument("-thr", "--threshold", type=float, default=0.1, help="[-1, thr) -> bus 1 / [thr, 1] -> bus 2")
    # SACD parameters
    ...
    parser.add_argument("-te", "--target_entropy_scale", type=float, default=0.98, help="coefficient for scaling the autotune entropy target")
    args = parser.parse_args()
    return args

def log_params(args, path, sampled_params):
    args_dict = vars(args).copy()
    args_dict.update(sampled_params)
...
def load_agent(args, my_dir, my_agent):
    ...
    with open(os.path.join(load_model_dir, "param.json"), "r", encoding="utf-8") as f:
        params = json.load(f)
        ...
        if params["case"] != args.case:
            raise Exception(f"Model of agent loaded is for case {params['case']}, but current agent is for case {args.case}")
        if params["forecast"] != args.forecast:
            raise Exception(f"Model of agent loaded contains {params['forecast']} steps of forecast, but current agent has {args.forecast} steps")
...
if __name__ == "__main__":
    ...
    kwargs = vars(args)
    sampled_params = sample_params(None, args.agent, args.middle_agent)
    kwargs.update(sampled_params)
    log_params(args, output_result_dir, sampled_params)
    if torch.cuda.is_available():
        print(">> >> using cuda")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # Define environments
    env, test_env, env_path = make_envs(args)
    # Select chronics and define dn_ffw
    train_chronics, valid_chronics, _, dn_ffw, ep_infos = select_chronics(env_path, env, test_env, args.case)
    # Specify agent
    my_agent = make_agent(env, env_path, kwargs)
    best_score = -100
    if args.load_agent:

...
def get_sub_adjacency_matrix(line_to_sub_id_tuple: Tuple[List, List], rho: np.array = None,
    ...
    adj = from_edge_list(edge_list, directed=False).toarray()
...
def get_sub_id_to_elem_id(rllib_env) -> Tuple[Dict[int, List[int]], Dict[int, List[Tuple[int,int]]], Dict[int, List[Tuple[int,int]]]]:
    """
    Get substation to element id mapping for all substations
    as well as element to action mapping for topology actions
    and action to topology mapping for topology actions.

    Parameters:
    -----------
    rllib_env: HierarchicalGridGym

    Returns:
    --------
    sub_id_to_elem_id: Dict[int, List[int]]
    element_to_action_num: Dict[int, List[Tuple[int,int]]]
    action_to_topology: Dict[int, List[Tuple[int,int]]]

    """
    sub_id_to_elem_id = defaultdict(list)
    for i in range(rllib_env.org_env.n_sub):
        sub_id_to_elem_id[i] = list(rllib_env.org_env.grid_objects_types[rllib_env.org_env.sub_info == i, 0])

    element_to_action_num = defaultdict(list)
    action_to_topology = defaultdict(list) # action_num -> List[Tuple(element_id, bus_bar)]
    
    for act_num,act in enumerate(rllib_env.all_actions_dict):
        # We are only interested in topology modification actions
        if not hasattr(act, 'impact_on_objects'):
            continue

        topology_changes = act.impact_on_objects().get("topology", {}).get("assigned_bus", [])
        if not topology_changes:
            continue

        # Assumes single substation is modified at a time
        sub_modified = topology_changes[0]["substation"] 
        
        modified_buses = [topo_change["bus"] for topo_change in topology_changes]
        # Get the element IDs (e.g., lines, generators) associated with the modified substation
        element_ids_in_sub = np.where(rllib_env.org_env.sub_info == sub_modified)[0]

        for element_id, bus in zip(element_ids_in_sub, modified_buses):
            # Map element ID to the action number and the bus it connects to
            element_to_action_num[element_id].append((act_num, bus))
            # Map action number to the element ID and the bus it connects to
            action_to_topology[act_num].append((element_id, bus))

    return sub_id_to_elem_id, element_to_action_num, action_to_topology

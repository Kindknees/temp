...
class ParamTuningTrain:
    def __init__(self, agent, env, test_env, dn_ffw, ep_infos, trial=None, seed=0):
        self.agent = agent
        self.env = env
        self.test_env = test_env
        self.dn_ffw = dn_ffw
    ...
    def train(self, seed, nb_steps, eval_steps, train_chronics, valid_chronics, output_dir, model_path, max_ffw, best_score=-100, verbose=False):
        ...
        while step < nb_steps:
            ...
            while not done and step < nb_steps:
                action = self.agent.agent_act(obs, is_safe=True, sample=True)
                next_obs, reward, done, info = self.env.step(action)
                self.agent.save_transition(reward, done)
                ...
                if step % eval_steps == 0:
                    eval_score = self.evaluate(valid_chronics, max_ffw)
                    self.last_mean_reward = eval_score
                    if verbose:
                        print(f"Step {step}, Eval Score: {eval_score}")
                    ...
                    if eval_score > best_score:
                        ...
                        self.agent.save_model(model_path, f"best_model")
        ...
        self.agent.save_model(model_path, f"final_model")
        return best_score

    def evaluate(self, chronics, max_ffw):
        scores = []
        for chronic in chronics:
            ...
            total_reward = 0
            steps = 0
            while not done and steps < max_ffw:
                action = self.agent.agent_act(obs, is_safe=True, sample=False)

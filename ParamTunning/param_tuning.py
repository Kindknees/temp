...
def cli():
    ...
    parser.add_argument("-d", "--dir", type=str, default="..", help="change the directory used for storing environment + data")
    ...
    parser.add_argument("-ns", "--nb_steps", type=int, default=200, help="the total number of agent steps")
    parser.add_argument("-ev", "--eval_steps", type=int, default=25, help="the number of steps between each evaluation")
    parser.add_argument("-m", "--mask", type=int, default=3, help='this agent manages the substations containing topology elements over "mask"')
    parser.add_argument("-mr", "--max_reward", type=int, default=10, help="max reward during training")
    parser.add_argument("-i", "--input", type=str, nargs="+", default=["p_i", "r", "o", "d", "m"], 
    ...
    parser.add_argument("-dg", "--danger", type=float, default=0.9, help="the powerline with rho over danger is regarded as hazardous")
    parser.add_argument("-ma", "--middle_agent", type=str, default="ppo", choices=["fixed", "random", "capa", "ppo"])
    parser.add_argument("-a", "--agent", type=str, default="ppo", 
                       choices=["sac", "sac2", "sacd", "sacd2", "masacd", "dsacd", "dqn", "dqn2", "ppo", "mappo", "dppo"])
    parser.add_argument("-hn", "--head_number", type=int, default=8, help="the number of head for attention")
    parser.add_argument("-sd", "--state_dim", type=int, default=128, help="dimension of hidden state for GNN")
    ...
    parser.add_argument("-tr", "--n_trials", type=int, default=4, help="Maximum number of trials for param tuning")
    parser.add_argument("-nj", "--n_jobs", type=int, default=1, help="Number of jobs to run in parallel")
    parser.add_argument("-st", "--n_startup_trials", type=int, default=3, help="Stop random sampling after N_STARTUP_TRIALS")
    parser.add_argument("-ws", "--n_warmup_steps", type=int, default=3, 
                       help="Number of steps before evaluations during the training. -> Pruning is disabled until the trial exceeds the given number of step.")
    parser.add_argument("-to", "--time_out", type=int, default=5, help="number of hours before time out")
    args = parser.parse_args()
    return args

def trial_one_seed(env, env_path, kwargs, test_env, dn_ffw, ep_infos, seed, train_chronics, valid_chronics, output_result_dir, model_path, trial, best_score):
    ...
    best_score = trainer.train(seed, nb_steps, eval_steps, train_chronics, valid_chronics, output_result_dir, model_path, get_max_ffw(case), best_score=best_score, verbose=False)
...
def objective(trial: optuna.Trial) -> float:
    ...
    middle_agent = args.middle_agent
    kwargs = vars(args)
    kwargs.update(sample_params(trial, agent_name, middle_agent))
    ...
    try:
        for seed in seeds:
            last_mean_reward[seed], best_score = trial_one_seed(
                env, env_path, kwargs, test_env, dn_ffw, ep_infos, seed, train_chronics, valid_chronics, output_result_dir, model_path, trial, best_score)
...
def save_optuna_visualizations(output_dir):
    from optuna.visualization.matplotlib import (
        plot_param_importances, plot_parallel_coordinate, plot_timeline, plot_intermediate_values, plot_contour)
    try:
...
if __name__ == "__main__":
    ...
    if args.load_study:
        ...
        try:
            study_id = storage.create_new_study([optuna.study.StudyDirection.MAXIMIZE], study_name=f"tuning_{args.agent}")
        ...
        study = optuna.load_study(sampler=sampler, pruner=pruner, study_name=f"tuning_{args.agent}", storage=storage)
    else:
        study = optuna.create_study(sampler=sampler, pruner=pruner, direction="maximize", study_name=f"tuning_{args.agent}")
    try:
        study.optimize(objective, n_trials=args.n_trials, n_jobs=args.n_jobs, timeout=args.time_out * 3600)
    except KeyboardInterrupt:
        pass
    report_trials(study, output_result_dir)
    save_optuna_visualizations(output_result_dir)
    plot_trials(output_result_dir, len(study.trials), args.seeds, args.nb_steps, name=f"{args.name}_{args.agent}")

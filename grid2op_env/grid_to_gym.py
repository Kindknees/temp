...
from grid2op_env.rewards import CombinedReward, ScaledL2RPNReward
...
def create_gym_env(**env_config: Any) -> tuple:
    ...
    if with_opponent:
        env = grid2op.make(env_name, reward_class=ScaledL2RPNReward, test=False, backend=backend, **opponent_kwargs)
    else:
        env = grid2op.make(env_name, reward_class=ScaledL2RPNReward, test=False, backend=backend)

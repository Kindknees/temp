...
import numpy as np
from Agents.l2rpn_base_agent import L2rpnAgent
from MultiAgents.MiddleAgent import PPOMiddleAgent
...
class IMARL(L2rpnAgent):    # Independent Multi-Agent Reinforcement Learning
    """
    Multi Agent for L2RPN.
    Each agent is responsible for one substation.
    Middle agent uses PPO to select substations.
    """

    def __init__(self, env, **kwargs):
        super().__init__(env, **kwargs)
        middle_agent = MIDDLE_AGENT[kwargs.get("middle_agent", "ppo")]
        self.sub_picker = middle_agent(self.action_converter.masked_sorted_sub, action_space=self.action_space, 
                                      state_dim=kwargs.get('state_dim', 128), device=self.device, **kwargs)
    ...
    def agent_act(self, obs, is_safe, sample, dn_count=0) -> BaseAction:
        if not is_safe or (len(self.sub_picker.subs_2_act) > 0):
            with torch.no_grad():
                ...
                sub_2_act, (action, log_prob) = self.sub_picker.pick_sub(stacked_state, sample)
                low_level_action = self.agents[sub_2_act].produce_action(stacked_state, adj, sample=sample)
                goal = (sub_2_act, low_level_action)
                act = self.action_converter.plan_act(goal, obs.topo_vect)
                if sample:
                    self.update_goal(goal)
                    # Save middle agent transition data
                    value = self.sub_picker.critic(stacked_state, adj=None).item()
                    self.sub_picker.save_transition(stacked_state, action, 0, stacked_state, False, log_prob, value)
                elif (act == self.action_space()) & (dn_count < len(self.sub_picker.subs)):
    ...
    def save_transition(self, reward, done, n_step=1):
        ...
        # Update middle agent transition with reward and done
        if self.memory:
            last_transition = self.sub_picker.memory[-1]
            self.sub_picker.memory[-1] = last_transition._replace(reward=reward, done=done, next_state=next_state)

    def check_start_update(self):
        agent_mem_sizes = [len(agent.memory) for agent in self.agents.values()]
        if max(agent_mem_sizes) >= self.update_start or len(self.sub_picker.memory) >= self.sub_picker.update_start:
            return True
        return False
...
class DepMARL(IMARL):
    def update(self):
        ...
        for agent in self.agents.values():
            if len(agent.memory) >= self.update_start:
                agent.dependent_update(self.agents.values(), None)  # No transition probs needed
        self.sub_picker.update()
